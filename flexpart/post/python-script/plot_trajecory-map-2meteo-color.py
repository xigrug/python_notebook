...
filename_100="trajectories.txt"
data=pd.read_table(filename_100,sep='\s+',header=None,skiprows=[0,1,2,3,4,5])#数量不定的空格符可以用正则表达式\s+表示
lon=data.iloc[:,2]
lat=data.iloc[:,3]
height=data.iloc[:,4]
#lon1=data1.iloc[:,2]
#lat1=data1.iloc[:,3]
#height1=data1.iloc[:,4]
#lon2=data2.iloc[:,2]
#lat2=data2.iloc[:,3]
#height2=data2.iloc[:,4]
...
plt.colorbar(lc,cax)
#plt.show()
plt.savefig('traject-1line.png',dpi=500)
