...
def plotMap1():
    ...
    #Zoom in
    ax.set_extent([60, 138, 18, 70])# TaZhong:83.63,39.04
    #Add map features
...
for i in range(10,15):
    day=str(i)
    dataFile = '/public/home/hysplit/data/ecwmf/ERA-Int_5pl_201705'+day+'_20-70_60-145.nc'
#Open the dataset and print out metadeta
    ds = xr.open_dataset(dataFile)
    lat = ds['latitude']
    lon = ds['longitude']
    lons, lats = np.meshgrid(lon, lat)
    ds.sel(time=('2017-05-'+day+'T00:00:00'), level=850)
    hgt=ds.z.sel(time=('2017-05-'+day+'T00:00:00'), level=850)/9.8/10
    tem=ds.t.sel(time=('2017-05-'+day+'T00:00:00'), level=850)-273.15
...
#gl.xlines = False
    gl.xlocator = mticker.FixedLocator([60,80,100,120,140,160])
...
#avor_levels = np.linspace(15e-5,60e-5, 10)
    ax.set_title('850-hPa Heights(dagpm) & Temperature($^\circ$C), Barbs(kt), 2017-05-'+day+' 00:00:00UTC', fontsize=20)

    plt.plot(118.8,32.0,'go', transform=ccrs.PlateCarree())
    plt.text(118.7,32.0, 'NJ', color='g',
...
#vrel = ds['v'].sel(level=850).isel(time=0).values*1.944
    urel=ds.u.sel(time=('2017-05-'+day+'T00:00:00'), level=850).values*1.944
    vrel=ds.v.sel(time=('2017-05-'+day+'T00:00:00'), level=850).values*1.944

#Plot the barbs
    ax.barbs(lon, lat, urel, vrel, regrid_shape=12, zorder=20, transform=ccrs.PlateCarree())

#plt.savefig(dataFile[3:23]+"_850hpaWind.png")
    plt.savefig(day+"00_850hpaWindnew-smoothHL.png",bbox_inches='tight')
