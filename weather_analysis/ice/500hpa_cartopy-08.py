...
#Open the dataset and print out metadeta
    ...
    ds.sel(time=('2017-05-'+day+'T00:00:00'), level=500)
    hgt=ds.z.sel(time=('2017-05-'+day+'T00:00:00'), level=500)/9.8/10
    tem=ds.t.sel(time=('2017-05-'+day+'T00:00:00'), level=500)-273.15    
...
#avor_levels = np.linspace(15e-5,60e-5, 10)
    ax.set_title('500-hPa Heights(dagpm) & Temperature($^\circ$C), Barbs(kt), 2017-05-'+day+' 00:00:00UTC', fontsize=20)
...
#vrel = ds['v'].sel(level=500).isel(time=0).values*1.944
    urel=ds.u.sel(time=('2017-05-'+day+'T00:00:00'), level=500).values*1.944
    vrel=ds.v.sel(time=('2017-05-'+day+'T00:00:00'), level=500).values*1.944

#Plot the barbs
    ax.barbs(lon, lat, urel, vrel, regrid_shape=12, zorder=20, transform=ccrs.PlateCarree())

#plt.savefig(dataFile[3:23]+"_500hpaWind.png")
    plt.savefig(day+"00_500hpaWindnew-smoothHL.png",bbox_inches='tight')
