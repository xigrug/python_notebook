...
#气温实况 
   wget "http://www.nmc.cn/publish/observations/hourly-temperature.html" -O html
   ...
   echo $file
   wget -S -c -N --tries=5 --wait=5 ${url} -O ${file}; sleep 5s
   rm -f html
...
#风场实况
   wget "http://www.nmc.cn/publish/observations/hourly-winds.html" -O html
...
echo $i
    echo ${pname[$i]}
    wget -S -c -N --tries=5 --wait=5 --no-proxy --no-check-certificate "http://www.nmc.cn/publish/satellite/fy2.htm" -O html
