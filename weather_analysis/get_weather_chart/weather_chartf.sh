#!/bin/bash
# -----------------------------------------------------
# AUTHOR: ZIQIANG JIANG, NANJING UNIVERSITY, 01/25/2016
# -----------------------------------------------------
...
#if [ "$Cur_hour" -eq "07" ]; then
# cd /data/forecast/statis/data/weather2
# mkdir $Cur_date
# cd $Cur_date
#pwd
#cp E* /data/forecast/statis/data/weather2/temp
#cp K* /data/forecast/statis/data/weather2/temp
#cp N* /data/forecast/statis/data/weather2/temp
#exit 0
#fi
...
#韩国 500温度和相对涡度   
   ...
   ln -sf  ${fname3} ${file3}
        if [ "`ls -l ${fname1} | awk '{print $5}'`" = "0" ]; then
		 ...
		 cp ${file1} /data/forecast/statis/data/weather2/temp
        fi
        if [ "`ls -l ${fname2} | awk '{print $5}'`" = "0" ]; then
         rm -f ${fname2}
		else
		 cp ${file2} /data/forecast/statis/data/weather2/temp		 
        fi
        if [ "`ls -l ${fname3} | awk '{print $5}'`" = "0" ]; then
         rm -f ${fname3}
		else
		 cp ${file3} /data/forecast/statis/data/weather2/temp		 
        fi
   i=$(($i+1))
   hour=`expr 6 + $hour`   
done
...
#美国 海平面气压和降水
   ...
   rm -f html
        if [ "`ls -l ${fname} | awk '{print $5}'`" = "0" ]; then
		 ...
		 cp ${file} /data/forecast/statis/data/weather2/temp		 
        fi
#美国 海平面气压和风场
