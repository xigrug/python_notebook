...
# make legend with dummy points
    for a in [100,500,1000,1500,2000,2500]:
        plt.scatter([], [], c='k', alpha=0.5, s=a/100+20,
                label=str(a)+'')
#plt.legend(scatterpoints=1, frameon=False,
#           labelspacing=1, loc='lower left',title= 'AQI',
#           bbox_to_anchor=(,0),ncol=5)
    plt.legend(scatterpoints=1, frameon=False,
           labelspacing=1, loc='lower left',title= 'PBLH',
           bbox_to_anchor=(0,-0.1,1,0.2),
           mode="expand", borderaxespad=0, ncol=6)
    plt.title(str(times)[0:10], fontsize=20)
    return fig, ax

for f in glob.glob("AUTO_5MINS_NPP_MicroPhys_201*necp.csv"):
...
#for f in glob.glob("AUTO_5MINS_NPP_MicroPhys_20150801_0442_Table.txt"):
    ...
    ccn_data = pd.read_csv(f, sep=',',skiprows=0,header=0,na_values=[-99,-9999])
    ...
    aqi = ccn_data['PBLH'].values
    ccn_max=np.nanmax(ccn)
    pblh_max=np.nanmax(aqi)
    pblh_min=np.nanmin(aqi)
    print(pblh_max,pblh_min)
    ...
    fig, ax= plotMap()
    plt.savefig('yrd_CCN_'+str(times)[0:10]+'_pblh.png',dpi=200)
    #plt.savefig('yrd_CCN_pblh.png',dpi=200)
