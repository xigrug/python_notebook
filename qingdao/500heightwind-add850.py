...
def wea(fname,figname,itime):
    ...
    for info, shp in zip(bm.states_info, bm.states):
        ...
        if proid == 'Shandong':
            x, y = zip(*shp)
            x1, y1 = bm(120.2,36.04)
            bm.plot(x, y, marker=None,color='g')
            bm.plot(x1, y1, marker='^', color='m', markersize=8, markeredgewidth=2)
    ...
    plt.clabel(contours, inline=1, fontsize=10, fmt="%i")
    plt.title('WRF Weather Forecast at %s\nAtmospheric Environment Research Center\nNanjing University'%figname[12:22])
...
def main():
    ...
    for i in range(len(list_name)-1):
        for itime in [0,12]:
            fname=list_name[i]
            print(fname)    
            figname = "500-850_wea_day_"+fname[58:68]+"_"+str(itime)
